...
def run_generate_cover_letter_task(context: ApplicationContext) -> str | None:
    """
    ...
    system_instruction, user_content = build_cover_letter_prompt(
        cv_data=context.cv_data.model_dump(), job_data=context.job_data.model_dump()
    )
    ...
    html_document = render_cover_letter_html(
        metadata=context.cv_data.identity.model_dump(), content=cover_letter_content.model_dump()
    )
