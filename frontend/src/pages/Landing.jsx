import { ArrowRight, FileText, SearchCheck, FolderKanban, Sparkles } from 'lucide-react';
  ...
  const features = [
    {
      ...
      description: 'Generate personalized cover letters tailored to every job posting.',
    },
    {
      icon: SearchCheck,
      title: 'Gap Analysis',
      description: 'Compare your CV against job requirements and identify missing skills.',
    },
    {
      icon: FolderKanban,
      title: 'Job Tracker',
      description: 'Organize every application and keep track of your progress.',
    },
  ];
    ...
    <main className="relative overflow-hidden">
      ...
      <section className="mx-auto flex min-h-[90vh] max-w-7xl flex-col items-center justify-center px-6 text-center">
        ...
        <h1 className="max-w-5xl text-5xl font-black leading-tight tracking-tight text-text-main md:text-7xl">
          Land Your Next Job <span className="bg-gradient-to-r from-primary to-primary-hover bg-clip-text text-transparent">Faster</span>
        </h1>

        <p className="mt-8 max-w-3xl text-lg leading-8 text-text-muted md:text-xl">
          Career Toolkit helps you generate tailored cover letters, analyze your CV against job requirements, and organize every application—all in one intelligent workspace.
        </p>

        <div className="mt-10 flex flex-wrap items-center justify-center gap-4">
          <button onClick={onGetStarted} className="group flex items-center gap-2 rounded-xl bg-primary px-7 py-4 font-semibold text-white transition hover:-translate-y-1 hover:bg-primary-hover">
            Get Started
            <ArrowRight size={18} className="transition group-hover:translate-x-1" />
          </button>

          <button className="rounded-xl border border-border bg-card-bg px-7 py-4 font-medium text-text-main transition hover:bg-input-bg">Explore Features</button>
        </div>
      ...
      <section className="mx-auto max-w-7xl px-6 pb-20">
        <div className="grid gap-6 md:grid-cols-3">
          {features.map((feature) => {
            ...
            return (
              <div key={feature.title} className="group rounded-2xl border border-border bg-card-bg p-8 transition hover:-translate-y-2 hover:border-primary/50">
                ...
                <p className="leading-7 text-text-muted">{feature.description}</p>
              </div>
            );
          })}
        </div>
      ...
      <section className="mx-auto max-w-7xl px-6 pb-28">
        <div className="rounded-3xl border border-border bg-card-bg p-8 shadow-2xl">
          <div className="mb-8 flex items-center justify-between">
            <div>
              <h2 className="text-2xl font-bold text-text-main">Your Career Dashboard</h2>

              <p className="mt-2 text-text-muted">Manage everything from one place.</p>
            </div>

            <div className="rounded-full bg-success/10 px-4 py-2 text-sm text-success">AI Ready</div>
          </div>
            ...
            <table className="w-full">
              ...
              <tbody>
                {[
                  ...
                  <tr key={row[0]} className="border-t border-border text-text-main">
